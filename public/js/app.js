document.addEventListener('DOMContentLoaded', () => {
    ...
    langSearch.addEventListener('input', (e) => {
        ...
        const filtered = allLanguages.filter(l => l.name.toLowerCase().includes(term));
        renderLangList(filtered);
    });

    closeLangModal.onclick = () => {
        langModalOverlay.style.display = 'none';
        if (customLangSelector) customLangSelector.value = 'en';
    };

    window.onclick = (e) => {
        if (e.target === langModalOverlay) {
            langModalOverlay.style.display = 'none';
        }
    };
